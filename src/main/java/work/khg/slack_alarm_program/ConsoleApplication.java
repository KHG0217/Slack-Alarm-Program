...
public class ConsoleApplication {
    ...
    public void sendTwitterStatusToSlack() {
        ...
        String checkTwitterStatusFText = checkTwitterAuth.checkTwitterStatusF();

        if(checkTwitterStatusFText != null) {
            slackApi.sendSlackText(webHookUrl,checkTwitterStatusFText);
        }

        List<String> ipList = checkTwitterAuth.selectIpList();
        String localIp = "175.125.218.16";
        ipList.remove(localIp);
        for (String ip: ipList) {
            String checkAvailableAuthText = checkTwitterAuth.checkAvailableAuth(ip);
            if (checkAvailableAuthText != null) {
                slackApi.sendSlackText(webHookUrl,checkAvailableAuthText);
            }
            String checkAvailableXctXidTokenText = checkTwitterAuth.checkAvailableXctXidToken(ip);
            if(checkAvailableXctXidTokenText != null){
                slackApi.sendSlackText(webHookUrl,checkAvailableXctXidTokenText);
            }
        }
    }
}
